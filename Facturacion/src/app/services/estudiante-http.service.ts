...
export class EstudianteHttpservice {
  ...
   'http://172.31.5.24:8080/WSSIIEPN/webresources/SilaboEPN/carga/${cedula}/${fecha}/${id}';
  ...
  getone(cedula:string,fecha:string,id:string): Observable<EstudianteModel[]>{
    return this.httpClient.get<EstudianteModel[]>(this.URL_API.replace('${cedula}',cedula).replace('${fecha}',fecha).replace('${id}',id))
  }
